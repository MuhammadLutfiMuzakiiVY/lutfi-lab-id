...
let EmailService = EmailService_1 = class EmailService {
    constructor(configService) {
        ...
        this.fromName = 'Lutfi-Lab.ID';
        this.fromAddress = this.configService.get('EMAIL_USER') || 'noreply@lutfi-lab.id';
            ...
            <tr>
                <td style="padding: 40px;">
                    ...
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top: 30px;">
                        <tr>
                            <td align="center">
                                <a href="${this.configService.get('CORS_ORIGIN') || 'http://localhost:52334'}/dashboard" style="display: inline-block; background: linear-gradient(135deg, #6366f1, #8b5cf6, #d946ef); color: #ffffff; text-decoration: none; padding: 16px 40px; border-radius: 12px; font-size: 16px; font-weight: 700; box-shadow: 0 10px 30px rgba(99, 102, 241, 0.4);">
                                    Mulai Sekarang →
                                </a>
                            </td>
                        </tr>
    ...
    async sendPasswordResetEmail(email, resetToken, name) {
        const resetUrl = `${this.configService.get('CORS_ORIGIN') || 'http://localhost:52334'}/reset-password?token=${resetToken}`;
        const content = `
            <tr>
                <td style="padding: 40px;">
                    <h2 style="margin: 0 0 15px; color: #ffffff; font-size: 24px; font-weight: 700; text-align: center;">
    ...
    async sendLoginAlertEmail(email, loginData, name) {
        const content = `
            <tr>
                <td style="padding: 40px;">
                    <h2 style="margin: 0 0 15px; color: #ffffff; font-size: 24px; font-weight: 700; text-align: center;">
    ...
    async sendAccountLockedEmail(email, unlockTime, name) {
        const content = `
            <tr>
                <td style="padding: 40px;">
                    <div style="text-align: center; margin-bottom: 30px;">
